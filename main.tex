...
\subsection{Blockchain accounts and states.}

\paragraph{Accounts.}
\def\accounts{\mathit{Accounts}}

An \emph{account} is a tuple of the form $$\mathit{account}:=(\mathit{external}\in\Bool,\Abi\in \Str\to \bytecodes, \store\in \Uint\to \vals)$$
where $\Abi$ is the account's application binary interface and $\store$ is the account's storage. $\Abi$ maps strings (function names) to bytecodes and $\store$ maps positions to values in a persistent manner.
We say that an account $A$ is \emph{external} if $A.\mathit{external}$ is true, and we say that $A$ is a \emph{contract} otherwise. We use $\accounts$ to denote the set of possible accounts and $\contracts$ to denote the subset of accounts that are contracts.

All account $A$ must satisfy the properties:
\begin{itemize}
\item if $A$ is external, then $A.\Abi(f)=\z_\bytecodes$ and $A.\store(p)=\z_\vals$ for all function name $f$ and position $p$. 
\item if $A$ is a contract, there exists a finite set of function names $\mc I_A\subseteq\Str$, such that for all $f\not\in \mc I_A$, $A.\Abi(f)=\revert{}$. We call the smallest set $\mc D_A$ satisfying the property the \emph{(effective) interface} of the contract.
\end{itemize}
