...
$$
% no takerWants variable (or similar)?
...
% should mention the 2-step maker->mgv, mgv->taker that means we can show we don't unduly penalize the taker (or the maker) when their counterparty is responsible for the transfer failure (eg theyve been blacklisted)
...
% This doesn't seem to be very clear (infer can mean guess right?)
\end{proposition}
...
% nonzero partial fill? otherwise you can imagine an offer that somehow gives 0 and reinserts itself, for instance by exploiting bad rounding in the Mangrove contract
\end{proposition}
