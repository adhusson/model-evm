...
We use $\accounts$ to denote the set of possible accounts. A \emph{contract} is an account $A$ for which there exists at least a function symbol $f$ such that $A.\Abi(f)\neq\z_\bytecodes$. The \emph{effective interface} of an account $A$ is a set of function symbols $I_A\subseteq\Str$ such that for all $f\not\in I_A$, $A.\Abi(f)=\revert{\str{not defined}}$ and for all $f\in I_A$, $A.\Abi(f)\neq\revert{\str{not defined}}$. When modelling a smart contract we only mention its effective interface, and assume that all other function symbol map to the reverting bytecode on the reserved error message $\str{not defined}$. Note that externally owned accounts can be modelled as accounts whose ABI map all symbols to $\z_\bytecodes$ and whose store maps all position to $\z_\vals$.
