...
$$
where \emph{outbound} and \emph{inbound} are the addresses of the ERC20 that manage respectively the inbound and outbound assets of the offer, \emph{price} is the Maker price at which it is willing to exchange tokens, and \emph{gasreq} is the amount of gas units that the Maker requires to execute its offer when called by Mangrove. In case two offers have the same price, the one that requires the least gas is considered best in the offer list. In case of strict equality, offers that were inserted first take precedence.

A call to Mangrove's \texttt{marketOrder} can be modelled as:
$$
\mathtt{marketOrder}(\mathit{outbound}, \mathit{inbound}, \mathit{limit_{price}}, \mathit{limit_{gasreq}})
$$
where \emph{outbound} and \emph{inbound} identifies the traded assets as before. The final arguments $\mathit{limit_{price}}$ and $\mathit{limit_{gasreq}}$ are respectively a price limit for the market order (see \autoref{sec:matching} and a bound on the amount of gas that can be spent on failing offers (see \autoref{sec:ftd}).

\subsection{Offer execution}\label{sec:matching}

Mangrove maps every offer, on an outbound/inbound market, to the address of the Maker that posted it. When a Taker triggers a market order transaction at at limit price $p$, each offer is 
